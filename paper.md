...
实验中以 naive 检索模式作为系统内基线，用于表示不引入图谱扩展的基础检索效果；local、global、hybrid 和 mix 模式用于观察图谱检索、向量召回和混合策略对证据召回的影响。系统内 naive 模式仍然使用已经完成 RAG-Anything/MinerU/VLM 处理后的知识库，因此它不同于原始文档直接分块的普通 RAG。为补充普通 RAG 对照，本文将原始 22 个测试文件直接进行文本抽取、普通分块和 BGE-M3 向量嵌入，写入独立 Qdrant collection `baseline_raw_text_vector_rag_bge_m3_1024d`，并使用同一生成模型 Qwen/Qwen3.5-122B-A10B 完成 85 个问题的回答。该 Text-only dense vector RAG 基线不使用 RAG-Anything 多模态解析、VLM 图片描述、LightRAG 实体关系抽取或 Neo4j 图谱检索。对于文本型问题，向量召回通常能够提供较稳定的语义匹配；对于涉及概念关系、先修关系和跨章节问题的样本，图谱辅助检索能够补充实体关系线索；对于图片和表格相关问题，多模态文本化描述进入索引后，可作为混合检索的证据来源。

**表 5-8 检索质量实测结果表**

| 检索模式 | 成功率 | Recall@1 | Recall@5 | MRR | nDCG@5 | 引用有效率 | 平均耗时/s | P95耗时/s |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Text-only dense vector RAG | 100% | 0.3571 | 0.6580 | 0.7024 | 0.6135 | 1.0000 | 8.81 | 11.13 |
...
从实测结果看，普通向量 RAG 基线和系统五种模式均能够完成 85 个问题的批量评估，接口成功率和引用有效率均达到 100%，说明各检索路径均能返回可追溯来源。在以标准文件名作为自动匹配依据的检索指标上，系统 naive 模式取得最高 Recall@5、MRR 和 nDCG@5；Text-only dense vector RAG 的 Recall@5 为 0.6580，低于系统 naive 的 0.7662，但平均耗时为 8.81s，P95 为 11.13s，长尾耗时低于系统内模式。该结果说明，普通向量检索能够作为较强文本基线，但 RAG-Anything/MinerU/VLM 处理后的系统 naive 索引在本数据集上进一步提升了文件级召回和排序质量。
